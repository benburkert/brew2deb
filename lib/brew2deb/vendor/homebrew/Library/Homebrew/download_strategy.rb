...
class CurlDownloadStrategy < AbstractDownloadStrategy
  ...
  def stage
    ...
    when /^PK\003\004/ # .zip archive
      ...
      chdir
    when /^\037\213/n, /^BZh/, /^\037\235/n  # gzip/bz2/compress compressed
    ...
    when 'Rar!'
      ...
      unless File.exists?(basename)
        FileUtils.ln @tarball_path, basename
      end
    end
  end
  ...
  def stage
    ...
    Dir.chdir @clone do
      ...
      if File.exist?('.gitmodules')
        ...
        sub_cmd = "git checkout-index -a -f \"--prefix=#{dst}/${toplevel##{Dir.pwd}}/$path/\""
